...
"module add --name=org.postgresql --resources=${WF_HOME}/modules/system/layers/base/org/postgresql/main/postgresql-42.7.8.jar --dependencies=jakarta.transaction.api,javax.api,java.naming,ibm.jakarta.jakartaee-api;" \
    ...
    "if (outcome == success) of /subsystem=datasources/data-source=PostgresDS:read-resource() then echo DS exists else /subsystem=datasources/data-source=PostgresDS:add(jndi-name=java:/jboss/datasources/PostgresDS, driver-name=postgresql, connection-url=jdbc:postgresql://${env.DB_HOST:${env.POSTGRES_HOST:postgres}}:${env.DB_PORT:5432}/${env.DB_NAME:persondb}?socketTimeout=5&connectTimeout=5, user-name=${env.DB_USER:person}, password=${env.DB_PASSWORD:person}, min-pool-size=5, max-pool-size=20, connection-timeout=5000, use-ccm=true, enabled=true) end-if;" \
